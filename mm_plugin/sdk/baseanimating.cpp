...
DEFINEVAR(CBaseAnimating, m_bSequenceFinished);

bool CBaseAnimating::Init(CGameConfig* config, char* error, size_t maxlength)
{
	try
	{
	...
	CBaseAnimating::offset_HandleAnimEvent = config->GetOffset("CBaseAnimating::HandleAnimEvent");
	if (CBaseAnimating::offset_HandleAnimEvent == -1)
	{
	...
	OFFSETVAR_DATA(CBaseAnimating, m_bSequenceFinished);
	...
	void* aVal = config->GetMemSig("GetAnimationEvent");
	if (aVal == nullptr)
	{
