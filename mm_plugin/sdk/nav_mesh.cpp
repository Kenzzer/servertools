...
bool CNavMesh::Init(CGameConfig* config, char* error, size_t maxlength)
{
	nav_solid_props = icvar->FindVar("nav_solid_props");

	uint8_t* addr = (uint8_t*)config->GetMemSig("CNavMesh::Load");
	if (addr)
	{
		int offset = config->GetOffset("TheNavMesh");
		if (offset == -1)
		{
		...
		offset = config->GetOffset("TheNavAreas");
		if (offset == -1)
		{
	...
	try
	{
		CNavMesh::offset_m_isLoaded = config->GetOffset("CNavMesh::m_isLoaded");
		if (CNavMesh::offset_m_isLoaded == -1)
		{
	...
	META_CONPRINTF("NavMesh: 0x%X NavAreas: 0x%X\n", TheNavMesh, pTheNavAreas);
