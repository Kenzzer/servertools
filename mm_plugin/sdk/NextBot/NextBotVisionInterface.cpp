...
extern ConVar* nb_debug_known_entities;
...
bool IVision::IsLineOfSightClearToEntity( const CBaseEntity *subject, Vector *visibleSpot ) const
{
	...
	UTIL_TraceLine( GetBot()->GetBodyInterface()->GetEyePosition(), ((CBaseEntity*)subject)->WorldSpaceCenter(), MASK_BLOCKLOS_AND_NPCS|CONTENTS_IGNORE_NODRAW_OPAQUE, &filter, &result );
	Assert( result.DidHit() != bClear );
	if ( subject->IsPlayer() && ! bClear )
	{
		UTIL_TraceLine( GetBot()->GetBodyInterface()->GetEyePosition(), ((CBaseEntity*)subject)->EyePosition(), MASK_BLOCKLOS_AND_NPCS|CONTENTS_IGNORE_NODRAW_OPAQUE, &filter, &result );
...
{
	return IsLookingAt( ((CBaseCombatCharacter*)actor)->EyePosition(), cosTolerance );
}