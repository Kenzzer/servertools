...
#include "sdk/basepropdoor.hpp"
#include "sdk/nav.hpp"
#include "sdk/util.hpp"
#include "NextBot.h"
#include "NextBotGroundLocomotion.h"
#include "NextBotUtil.h"
#include "sdk/ndebugoverlay.hpp"
...
void NextBotGroundLocomotion::Reset( void )
{
	...
	m_nextBot = GetBot();
	m_nextBotChar = static_cast<NextBotCombatCharacter*>(m_nextBot->GetEntity());
...
void NextBotGroundLocomotion::Update( void )
{
	...
	m_nextBotChar->SetAbsVelocity( m_velocity );
...
bool NextBotGroundLocomotion::DetectCollision( trace_t *pTrace, int &recursionLimit, const Vector &from, const Vector &to, const Vector &vecMins, const Vector &vecMaxs )
{
	...
	if ( them && them->ShouldTouch( m_nextBotChar ) )
	{
		/// @todo construct mirror of trace
		them->OnContact( m_nextBotChar );
	}
	else
	{
	...
	while( true )
	{
		...
		if ( trace.startsolid )
		{
			...
			if ( trace.m_pEnt && !trace.m_pEnt->IsWorld() )
			{
				...
				if (!FClassnameIs(trace.m_pEnt, "prop_door_rotating"))//if ( dynamic_cast< CPhysicsProp * >( trace.m_pEnt ) != NULL && dynamic_cast< CBasePropDoor * >( trace.m_pEnt ) == NULL )
				{
...
void NextBotGroundLocomotion::UpdatePosition( const Vector &newPos )
{
	...
	if ( NextBotStop->GetBool() || (m_nextBotChar->GetFlags() & FL_FROZEN) != 0 || newPos == m_nextBot->GetPosition() )
	{
		return;
	}
...
void NextBotGroundLocomotion::UpdateGroundConstraint( void )
{
	...
	trace_t ground;
	NextBotTraceFilterIgnoreActors filter( m_nextBotChar, body->GetCollisionGroup() );
	...
	if ( ground.fraction < 1.0f )
	{
		...
		{
			// just landed
			m_nextBotChar->SetGroundEntity( ground.m_pEnt );
	...
	else
	{
		...
		if ( IsOnGround() )
		{
			GetBot()->OnLeaveGround( m_nextBotChar->GetGroundEntity() );
...
{
	return (m_nextBotChar->GetGroundEntity() != NULL);
}
...
{
	m_nextBotChar->SetGroundEntity( NULL );
	m_ground = NULL;
...
void NextBotGroundLocomotion::DescendLadder( const CNavLadder *ladder, const CNavArea *dismountGoal )
{
	...
	if ( body )
	{
		...
		QAngle angles = m_nextBotChar->GetLocalAngles();
		angles.y = ladderYaw;
		
		m_nextBotChar->SetLocalAngles( angles );
...
{
	return IsClimbingOrJumping() && (m_nextBotChar->GetAbsVelocity().z > 0.0f);
}
