...
extern ConVar* NextBotPathDrawSegmentCount;
...
bool Path::BuildTrivialPath( INextBot *bot, const Vector &goal )
{
	...
	CNavArea *startArea = ToolsNavMesh->GetNearestNavArea( start );
	if (startArea == NULL)
		return false;

	CNavArea *goalArea = ToolsNavMesh->GetNearestNavArea( goal );
	if (goalArea == NULL)
		return false;
